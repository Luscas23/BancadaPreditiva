...
void vibracaoInit() {
  ...
  // Melhoria 3 — detecta estado de repouso do sensor
  sw420_1_repouso = digitalRead(PIN_SW420_1);
  sw420_2_repouso = digitalRead(PIN_SW420_2);

  // Corrigido v5: CHANGE causava duplo disparo por vibração
  attachInterrupt(digitalPinToInterrupt(PIN_SW420_1), ISR_vibr1, RISING);
  attachInterrupt(digitalPinToInterrupt(PIN_SW420_2), ISR_vibr2, RISING);
}
